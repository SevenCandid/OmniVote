...
export PGPASSWORD=Nexra2026
psql -h omnivote-postgres -U postgres -d omnivote -c "UPDATE identity_users SET status = 'active' WHERE email = '$EMAIL';"

echo "Logging in..."
TOKEN=$(curl -s -X POST "http://localhost:8000/api/v1/identity/auth/login" \
     -H "Content-Type: application/x-www-form-urlencoded" \
     -d "username=$EMAIL&password=password123" | python -c "import sys, json; print(json.load(sys.stdin)['access_token'])")
