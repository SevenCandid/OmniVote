...
describe('MemberList Component', () => {
  it('displays loading state', () => {
    const { container } = renderWithProviders(
      <MemberList members={[]} isLoading={true} error={null} />
    );
    expect(
  ...
  it('displays empty state', () => {
    renderWithProviders(
      <MemberList members={[]} isLoading={false} error={null} />
    );
  ...
  it('displays error state', () => {
    ...
    expect(
      screen.getByText('Failed to load members: Test error')
    ).toBeInTheDocument();
  });
  ...
  it('displays members', () => {
    ...
    renderWithProviders(
      <MemberList members={members as any} isLoading={false} error={null} />
    );
...
describe('InviteMemberDialog Component', () => {
  ...
  it('calls invite mutation on valid submit', async () => {
    const mockInvite = vi
      .fn()
      .mockResolvedValue({ invitation_token: 'mock-token' });
    ...
    fireEvent.change(getByPlaceholderText(/recipient's email/i), {
...
describe('UserInvitationsPage', () => {
  ...
  it('displays user pending invitations', () => {
    ...
    vi.spyOn(membershipHooks, 'useAcceptInvitation').mockReturnValue({
      mutateAsync: vi.fn(),
      isPending: false,
    } as any);
    vi.spyOn(membershipHooks, 'useDeclineInvitation').mockReturnValue({
      mutateAsync: vi.fn(),
      isPending: false,
    } as any);
