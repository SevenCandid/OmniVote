...
export function SessionsPage() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setSessions([
        { id: '1', ip_address: '192.168.1.1', user_agent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36', last_active_at: new Date().toISOString(), is_current: true },
        { id: '2', ip_address: '10.0.0.5', user_agent: 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15', last_active_at: new Date(Date.now() - 86400000).toISOString(), is_current: false }
      ]);
      setIsLoading(false);
    }, 500);
  }, []);
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="bg-white dark:bg-[#18181B] border border-[var(--color-border-default-light)] dark:border-[var(--color-border-default-dark)] rounded-xl overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-[var(--color-neutral-secondary-light)]">Loading sessions...</div>
        ) : (
                ...
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0">
                    <svg className="w-5 h-5 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      {session.user_agent.includes('Mobile') || session.user_agent.includes('iPhone') ? (
